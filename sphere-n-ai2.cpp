...
std::vector<double> get_tp(int n) {
    ...
    std::transform(x.begin(), x.end(), sine_n_minus_1.begin(), [&](double xi) { return std::pow(std::sin(xi), n - 1); });
    std::vector<double> numerator(x.size());
    std::transform(tp_minus2.begin(), tp_minus2.end(), sine_n_minus_1.begin(), numerator.begin(), [&](double tp, double sine_n_minus_1) { return (n - 1) * tp + -std::cos(xi) * sine_n_minus_1; });
...
public:
    SphereN(int n, const std::vector<int>& base) {
        ...
        if (n == 2) {
            ...
            s_gen = std::make_unique<SphereN>(SphereN(n - 1, std::vector<int>(base.begin() + 1, base.end())));
        }
        this->n = n;
        auto tp = get_tp(n);
    ...
    std::vector<double> pop() {
        ...
        std::transform(s_gen_pop.begin(), s_gen_pop.end(), res.begin(), [&](double x) { return x * sinphi; });
        res.back() = std::cos(xi);
        return res;
    }
private:
