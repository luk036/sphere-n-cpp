...
std::vector<double> linspace(double start, double end, int num) {
    ...
    std::generate(result.begin(), result.end(), [&]() { double val = start; start += step; return val; });
    return result;
}

std::vector<double> neg_cosine = []() {
    std::vector<double> result = linspace(0.0, M_PI, 300);
    std::transform(result.begin(), result.end(), result.begin(), [](double x) { return -std::cos(x); });
    return result;
}();

std::vector<double> sine = []() {
    std::vector<double> result = linspace(0.0, M_PI, 300);
    std::transform(result.begin(), result.end(), result.begin(), [](double x) { return std::sin(x); });
    return result;
}();
...
std::vector<double> get_tp(int n) {
    ...
    std::transform(tp_minus2.begin(), tp_minus2.end(), sine.begin() + n - 1, result.begin(), [&](double tp, double sin_n_minus_1) {
...
class SphereN : public Sphere {
    ...
    double range;
public:
    SphereN(int n, std::vector<int> base) : Sphere(std::vector<int>(base.begin() + 1, base.begin() + 3)), n(n) {
        assert(n >= 2);
        vdc = Vdcorput(base[0]);
        if (n > 2) {
    ...
    std::vector<double> pop() {
        ...
        std::vector<double> result(s.size() + 1);
        std::transform(s.begin(), s.end(), result.begin(), [&](double s_i) { return xi * sinphi * s_i; });
...
double discrep_2(const std::vector<std::array<int, 3>>& K, const std::vector<std::vector<double>>& X) {
    int nsimplex = K.size();
    int n = K[0].size();
    double maxq = 0.0;
    double minq = 1000.0;
